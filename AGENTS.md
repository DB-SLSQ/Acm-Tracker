...
- **每天一套题要「从简单到难」**：题按打分排完就直接按天发下去的话，前几天全是这一阶段里
  最简单的题。实测（1635 分、目标 1900）第一阶段区间 1500~1900，前十天却是 1500/1500/1500…，
  平均每天难度跨度只有 6 分，等于天天秒签到题。改成按难度升序再「发牌」
  （`lib/plan.js` 的 `orderByDailyLadder`）之后平均跨度 133 分，每天都是一条从易到难的坡。
