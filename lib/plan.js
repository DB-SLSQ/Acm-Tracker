...
function scoreCandidate(problem, band, weakWeights) {
  const [lo, hi] = band;
  ...
  return weakScore + popularity + fit;
}
 ...
 */
function pickProblems(candidates, count, band, weakWeights, exclude) {
  ...
  const ranked = candidates
    ...
    .map((problem) => ({ problem, score: scoreCandidate(problem, band, weakWeights) }))
...
export function buildPlan({ user, solved, attempted, problems, target, weekly }) {
  ...
  const solvedProblems = rated.filter((problem) =>
    solved.has(problemKey(problem.contestId, problem.index)),
  );
  ...
  const axes = buildKnowledgeProfile(solvedProblems, tagProfile);
  ...
  for (let index = 1; index <= stages; index += 1) {
    ...
    const picked = pickProblems(candidates, count, band, weakWeights, assigned);
