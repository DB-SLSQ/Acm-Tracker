 ...
 *
 * 现在每天的位置都对着一个难度档（见 LADDER_TIER_OFFSETS），发到哪个位置就取
 * 剩下的题里离那一档最近的那道。一天四题就是四档各一道；一天两题就顺着往下一档挪，
 * 四天一轮覆盖两遍，长期下来每档题量一样多，四档的平均正好落在目标分上。
 */
function orderByDailyLadder(picked, perDay, tiers) {
  const pool = [...picked];
  if (pool.length <= 1) return pool;
  const ladder = [...(tiers ?? [])].sort((a, b) => a - b);
  const byRating = (a, b) => (a.rating ?? 0) - (b.rating ?? 0);
  if (!ladder.length) return pool.sort(byRating);

  const perDayCount = Math.max(1, perDay);
  const days = Math.max(1, Math.ceil(pool.length / perDayCount));
  const ordered = [];

  // 每天的第 slot 个位置对着哪一档：一天四题就是四档各一道（步长 1）；
  // 一天两题则拉开取（跨两档）——每天都是「一道能做的 + 一道要够一够的」，
  // 不会出现某一天两道都做不动。四天一轮下来每档的题量一样多。
  const tierStep = Math.max(1, Math.round(ladder.length / perDayCount));
  for (let day = 0; day < days && ordered.length < pool.length; day += 1) {
    const row = [];
    for (let slot = 0; slot < perDayCount; slot += 1) {
      if (ordered.length + row.length >= pool.length) break;
      const target = ladder[(day + slot * tierStep) % ladder.length];
      // 找剩下这些题里离这一档最近的那道
      let bestIndex = -1;
      let bestGap = Infinity;
      pool.forEach((problem, index) => {
        const gap = Math.abs((problem.rating ?? 0) - target);
        if (gap < bestGap) {
          bestGap = gap;
          bestIndex = index;
        }
      ...
      row.push(pool.splice(bestIndex, 1)[0]);
    }
    // 一天之内从易到难
    row.sort(byRating);
    ordered.push(...row);
  ...
  return ordered.concat(pool.sort(byRating));
}
...
function pickProblems(
  ...
  // 每个方向内部也按四档轮流排：这样这个方向的名额会均匀铺在四个难度上
  const orderedBuckets = new Map(
    [...buckets].map(([axis, list]) => [axis, interleaveByTier(list, tiers)]),
  );
...
export function buildPlan({
  ...
  for (let index = 1; index <= stages; index += 1) {
    ...
    // 区间要装得下每日阶梯的四档（最高一档在 center+450），所以上界抬到 +500。
    ...
    stageList.push({
      ...
      problems: orderByDailyLadder(picked, ladderPerDay, tiers).map(toClientProblem),
      review,
    });
  }
