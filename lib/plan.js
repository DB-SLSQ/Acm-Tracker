 ...
 * 把这一阶段的题排成「每天从简单到难」。
 *
 * 以前是把打分排序的结果直接按天发下去，于是前几十天全是这一阶段里最简单的那批题。
 * 实测 DB_SLSQ（1635 分，目标 1900）：第一阶段区间是 1500~1900，
 * 前 10 天却是 1500 / 1500 / 1500 / 1500 / 1600……平均每天难度跨度只有 6 分，
 * 等于天天在秒签到题；这一阶段里 1900 的题要等到几周以后才轮得到。
 *
 * 现在先按难度升序，再「发牌」：这一阶段的题摊到 d 天（d = 题数 ÷ 每天题量），
 * 第 i 天拿第 i、i+d、i+2d 张，于是每天都是从这一阶段的低端走到高端。
 * 各方向的题量配额不受影响，换的只是发放顺序。
 */
function orderByDailyLadder(picked, perDay) {
  const sorted = [...picked].sort((a, b) => (a.rating ?? 0) - (b.rating ?? 0));
  const days = Math.max(1, Math.ceil(sorted.length / Math.max(1, perDay)));
  if (days <= 1) return sorted;

  const ordered = [];
  for (let slot = 0; slot < days; slot += 1) {
    for (let index = slot; index < sorted.length; index += days) ordered.push(sorted[index]);
  }
  return ordered;
}
