...
export function deriveProgress(submissions) {
  ...
  // 通过的题要从 attempted 里摘掉。失败记录是先写进去的，通过之后再留着，
  // 「补题队列」和「做了没过的题」就会把早就补过的题一直算在里面
  // （实测一份 1245 条提交的记录：208 道没过的题里有 179 道其实已经通过了）。
  for (const key of solved.keys()) attempted.delete(key);
