// 其他 OJ 平台的数据抓取。
//
// 设计原则：只发「一个请求就能拿到」的东西，不为了凑数据去翻页。
// 牛客拿的是汇总数字；洛谷的练习页本身就把做过的题整个放在一个 JSON 里，
// 顺手把逐题列表也带上（训练计划用它排除做过的题），不会多一个请求。
 ...
 */
export async function fetchLuoguText(url, { timeoutMs = 20000 } = {}) {
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), timeoutMs);
...
export async function fetchLuogu(uid) {
  ...
  const difficulty = {};
  const cfProblems = [];
  // 逐题列表用来给训练计划「排除做过的题」。练习页没有时间信息，
  // 所以这些记录只进「哪些题做过了」，不进热力图和做题记录（那边要按天排）。
  const solved = [];
  for (const item of passed) {
    const level = Number(item?.difficulty ?? 0);
    difficulty[level] = (difficulty[level] ?? 0) + 1;
    const pid = typeof item?.pid === 'string' ? item.pid : '';
    const mirror = pid.match(/^CF(\d+)([A-Z]\d?)$/);
    if (mirror) cfProblems.push(`${mirror[1]}-${mirror[2]}`);
    if (pid) {
      solved.push({ pid, name: String(item?.name ?? ''), difficulty: level });
    }
  }
  ...
  return {
    ...
    stats,
    solved: stats['通过题目'] ?? 0,
    // solved 只是顺手带出来给同步流程用的，落库前会删掉（几千道题的 JSON 太大）
    extra: { difficulty, cfProblems, difficultyLabels: LUOGU_DIFFICULTY, solved },
    url,
  };
}
