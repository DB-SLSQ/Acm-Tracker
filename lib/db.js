...
export function savePlatformStats({ platform, account, nickname, stats }) {
  db.prepare(`
    INSERT INTO platform_stats (platform, account, nickname, stats, fetched_at)
    VALUES (?, ?, ?, ?, ?)
    ON CONFLICT(platform, account) DO UPDATE SET
      nickname = excluded.nickname, stats = excluded.stats, fetched_at = excluded.fetched_at
  `).run(platform, account, nickname ?? null, JSON.stringify(stats ?? {}), Date.now());
}

export function listPlatformStats() {
  return db
    .prepare('SELECT platform, account, nickname, stats, fetched_at FROM platform_stats ORDER BY platform')
    .all()
    .map((row) => ({
