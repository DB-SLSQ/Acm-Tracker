...
export function getProblemsByKeys(keys) {
  ...
  const select = db.prepare(
    'SELECT contest_id, idx, name, rating, tags, solved_count, type FROM problems WHERE contest_id = ? AND idx = ?',
  );
...
export function savePlanSnapshot(handleKey, target, signature, stageKeys) {
  ...
  `).run(handleKey, target, signature, Date.now(), JSON.stringify(stageKeys));
}
